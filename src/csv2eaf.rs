...
pub fn run(args: &clap::ArgMatches) -> std::io::Result<()> {
    ...
    if eaf_path.exists() {
        if !acknowledge(&format!("{} already exists. Overwrite? (y/n)", eaf_path.display()))? {
            println!("User aborted writing ELAN-file");
            std::process::exit(1)
        }
    }

    match eaf.write(&eaf_path, Some(4)) {
        Ok(()) => println!("Wrote {}", eaf_path.display()),
